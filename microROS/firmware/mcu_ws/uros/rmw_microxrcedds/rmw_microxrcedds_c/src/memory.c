  ...
  rmw_uxrce_mempool_t * mem)
{
  ...
  #if 0
    RMW_SET_ERROR_MSG("allow list\r\n");
    int i=0;
  struct rmw_uxrce_mempool_item_t * p;
  p = mem->freeitems;
  while (1)
  {
      if (p != NULL)
      {
          RMW_SET_ERROR_MSG("mem->freeitems[%d]= %d\r\n", i++, p);
          p = p->next;
      }
      else
      {
  ...
  i = 0;
  p = mem->allocateditems;
  while (1)
  {
      if (p != NULL)
      {
          RMW_SET_ERROR_MSG("mem->allocateditems[%d]= %d\r\n", i++, p);
          p = p->next;
      }
      else
      {
